...
public class DirectoryTreeListerHandler : IFileOperationHandler
{
    private readonly ILogger _logger;
    private IFileOperationHandler? _nextHandler;

    public DirectoryTreeListerHandler(ILogger logger)
    {
    ...
    public void Handle(string command, string? argument, string? secondArgument, FileStream? connectedFileStream)
    {
        if (command == "list")
        {
            var directoryTreeLister = new DirectoryTreeLister(_logger);
            if (argument != null) directoryTreeLister.Execute(argument, secondArgument);
        }
        else
        {
